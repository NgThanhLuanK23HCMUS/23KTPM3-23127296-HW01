...
![Ảnh chụp tin tuyển dụng 1](./screenshots/req1/job-01.png)
...
![Ảnh chụp tin tuyển dụng 2](./screenshots/req1/job-02.png)
...
![Ảnh chụp tin tuyển dụng 3](./screenshots/req1/job-03.png)
...
![Ảnh chụp tin tuyển dụng 4](./screenshots/req1/job-04.png)
...
![Ảnh chụp tin tuyển dụng 5](./screenshots/req1/job-05.png)
...
![Ảnh chụp tin tuyển dụng 6](./screenshots/req1/job-06.png)
...
![Ảnh chụp tin tuyển dụng 7](./screenshots/req1/job-07.png)
...
![Ảnh chụp tin tuyển dụng 8](./screenshots/req1/job-08.png)
...
![Ảnh chụp tin tuyển dụng 9](./screenshots/req1/job-09.png)
...
![Ảnh chụp tin tuyển dụng 10](./screenshots/req1/job-10.png)
...
**Mô tả:**  
CVE-2025-32711 là lỗi trong Microsoft 365 Copilot liên quan đến tấn công chèn lệnh vào prompt. Trong kịch bản tấn công, nội dung độc hại có thể được chèn vào dữ liệu mà Copilot xử lý, khiến hệ thống AI thực hiện hành vi ngoài ý muốn và có nguy cơ làm lộ thông tin nhạy cảm.
...
- Tổ chức cần kiểm tra Microsoft 365 admin center hoặc Service Health để xác nhận tenant đang dùng dịch vụ đã được cập nhật.
...
- **Năm công bố:** 2023
- **Loại lỗi:** Framework AI/LLM, SQL injection, rủi ro từ prompt sang câu lệnh SQL
...
- tác tử LLM có thể bị lợi dụng làm cầu nối tấn công vào cơ sở dữ liệu.
...
- **Năm công bố:** 2024
- **Loại lỗi:** Framework AI/LLM, tấn công chèn lệnh vào prompt, chèn truy vấn vào cơ sở dữ liệu đồ thị
- **Nguồn chính thống:** https://github.com/advisories/GHSA-45pg-36p6-83v9

**Mô tả:**  
`GraphCypherQAChain` có thể sinh Cypher query dựa trên prompt của người dùng. Attacker có thể dùng tấn công chèn lệnh vào prompt để khiến chain tạo truy vấn độc hại, dẫn đến đọc dữ liệu nhạy cảm, thay đổi dữ liệu trong graph cơ sở dữ liệu hoặc gây từ chối dịch vụ.
...
- Từ chối dịch vụ thông qua truy vấn tốn tài nguyên.
...
**Mô tả:**  
`PALChain` trong LangChain Experimental cho phép sinh và chạy Python mã để giải quyết bài toán. Các bản trước `0.0.306` có thể bị vượt qua cơ chế chặn import nguy hiểm, ví dụ thông qua `__import__`, dẫn đến khả năng thực thi mã tùy ý.
...
- Rủi ro đặc biệt cao nếu LLM tool chạy trong môi trường production.
- Cho thấy nguy cơ khi cho LLM quyền thực thi mã mà thiếu môi trường cách ly.
...
- Chạy thực thi công cụ trong môi trường cách ly tách biệt nếu bắt buộc phải dùng.
...
- **Năm công bố:** 2024
- **Loại lỗi:** Thiên kiến AI, lỗi tạo ảnh, điều chỉnh quá mức
- **Nguồn chính thống:** https://blog.google/products-and-platforms/products/gemini/gemini-image-generation-issue/

**Mô tả:**  
Google cho biết tính năng tạo ảnh người trong Gemini bị lỗi vì cơ chế tinh chỉnh nhằm tăng tính đa dạng đã không xử lý đúng các trường hợp cần phản ánh chính xác ngữ cảnh cụ thể, đặc biệt là ngữ cảnh lịch sử hoặc văn hóa. Model cũng trở nên quá thận trọng và từ chối một số prompt bình thường.
...
- Cải thiện đáng kể tính chính xác trước khi bật lại.
- Thực hiện kiểm thử mở rộng trước khi phát hành lại.
- Khuyến cáo không dựa hoàn toàn vào Gemini cho nội dung hiện sự kiện, tin tức đang phát triển hoặc chủ đề nhạy cảm.
...
- **Năm công bố:** 2024
- **Loại lỗi:** Lỗi cập nhật phần mềm bảo mật đầu cuối, gây sập hệ thống ở mức kernel
- **Nguồn chính thống:** https://www.crowdstrike.com/en-us/blog/falcon-content-update-preliminary-post-incident-report/

**Mô tả:**  
Ngày 19/07/2024, một bản cập nhật Rapid Response Content của CrowdStrike Falcon gây lỗi trên cảm biến Windows, dẫn đến màn hình xanh chết chóc. CrowdStrike cho biết lỗi liên quan đến bộ kiểm tra nội dung cho phép dữ liệu có vấn đề đi qua kiểm tra hợp lệ, sau đó bộ diễn giải nội dung gặp lỗi đọc bộ nhớ ngoài phạm vi cho phép.
...
- Làm nổi bật rủi ro của phần mềm bảo mật chạy ở mức kernel.

**Giải pháp khắc phục:**  

- Hoàn tác nội dung lỗi.
- Cải thiện Rapid Response Content testing.
- Bổ sung kiểm thử cục bộ của lập trình viên, kiểm thử khôi phục phiên bản, kiểm thử chịu tải, kiểm thử fuzzing, kiểm thử chèn lỗi, kiểm thử độ ổn định và kiểm thử giao diện.
- Thêm kiểm tra hợp lệ checks trong bộ kiểm tra nội dung.
- Cải thiện error handling trong bộ diễn giải nội dung.
- Triển khai triển khai theo từng giai đoạn và triển khai canary.
...
- **Năm công bố:** 2023
- **Loại lỗi:** SQL injection, thực thi mã từ xa, rò rỉ dữ liệu
- **Nguồn chính thống:** https://nvd.nist.gov/vuln/detail/CVE-2023-34362

**Mô tả:**  
MOVEit Transfer có lỗi SQL injection cho phép kẻ tấn công chưa xác thực truy cập cơ sở dữ liệu trái phép. Lỗ hổng này bị khai thác thực tế để cài web shell và đánh cắp dữ liệu từ nhiều tổ chức.
...
- **Năm công bố:** 2024
- **Loại lỗi:** Xâm nhập chuỗi cung ứng phần mềm, cửa hậu độc hại
- **Nguồn chính thống:** https://nvd.nist.gov/vuln/detail/CVE-2024-3094

**Mô tả:**  
Mã độc được phát hiện trong gói mã nguồn upstream của XZ Utils bắt đầu từ phiên bản `5.6.0`. Quá trình build `liblzma` bị chèn logic obfuscated để trích xuất tệp object độc hại từ test archive, sau đó sửa đổi một số function trong `liblzma`.
...
- Các bản phân phối Linux phải nhanh chóng quay lui phiên bản hoặc thay gói phần mềm.
...
- Cập nhật gói phần mềm từ repository chính thức của hệ điều hành.
...
- **Nguồn chính thống:** https://confluence.atlassian.com/doc/confluence-security-khuyến cáo bảo mật-2022-06-02-1130377146.html

**Mô tả:**  
Atlassian Confluence Server và Data Center có lỗi OGNL injection cho phép kẻ tấn công chưa xác thực thực thi mã tùy ý trên máy chủ. Lỗi có thể bị khai thác từ xa nếu Confluence được mở truy cập ra internet.
...
- **Năm công bố:** 2022
- **Loại lỗi:** Thực thi mã từ xa, ràng buộc dữ liệu không an toàn
- **Nguồn chính thống:** https://spring.io/security/cve-2022-22965

**Mô tả:**  
Spring Framework có lỗ hổng RCE liên quan đến ràng buộc dữ liệu trong một số điều kiện triển khai nhất định, đặc biệt với ứng dụng Spring MVC hoặc Spring WebFlux chạy trên JDK 9+ và deploy dạng WAR trên Servlet container như Tomcat.
...
- **Năm công bố:** 2022
- **Loại lỗi:** Nội suy chuỗi không an toàn, rủi ro thực thi mã từ xa
- **Nguồn chính thống:** https://commons.apache.org/proper/commons-text/security.html

**Mô tả:**  
Apache Commons Text trước `1.10.0` có rủi ro trong cơ chế nội suy chuỗi. Nếu ứng dụng dùng `StringSubstitutor` với dữ liệu đầu vào không tin cậy, kẻ tấn công có thể lợi dụng một số bộ nội suy nguy hiểm để gây thực thi mã hoặc truy xuất tài nguyên ngoài ý muốn.
...
- Có thể dẫn đến RCE trong một số cấu hình sử dụng.
- Tổ chức phải rà soát phụ thuộc thư viện phổ biến trong hệ sinh thái Java.
- Tạo nhầm lẫn lớn vì nhiều người so sánh với Log4Shell dù điều kiện khai thác khác nhau.

**Giải pháp khắc phục:**  

- Nâng cấp Apache Commons Text lên `1.10.0` hoặc mới hơn.
- Không truyền dữ liệu đầu vào không tin cậy vào `StringSubstitutor` nếu chưa validate/sanitize.
...
- **Năm công bố:** 2023
- **Loại lỗi:** Lộ thông tin nhạy cảm, rò rỉ mã phiên đăng nhập
- **Nguồn chính thống:** https://support.citrix.com/support-home/kbsearch/article?articleNumber=CTX579459

**Mô tả:**  
CitrixBleed là lỗi trong NetScaler ADC và NetScaler Gateway có thể làm lộ dữ liệu nhạy cảm trong bộ nhớ, bao gồm mã phiên đăng nhập. Nếu mã phiên đăng nhập bị lộ, kẻ tấn công có thể chiếm phiên đăng nhập mà không cần biết mật khẩu.
...
- **Năm công bố:** 2023
- **Loại lỗi:** Leo thang đặc quyền, thực thi mã từ xa
...
**Mô tả:**  
Cisco IOS XE giao diện Web UI có lỗi cho phép kẻ tấn công chưa xác thực tạo tài khoản có quyền cao trên thiết bị. Khi kết hợp với lỗi khác, kẻ tấn công có thể thực thi mã với quyền cao trên router/switch bị ảnh hưởng.
...
- **Năm công bố:** 2024
- **Loại lỗi:** Vượt qua xác thực, chèn lệnh hệ thống
- **Nguồn chính thống:** https://forums.ivanti.com/s/article/CVE-2023-46805-and-CVE-2024-21887

**Mô tả:**  
Ivanti Connect Secure và Ivanti Policy Secure có chuỗi lỗi gồm authentication vượt qua và chèn lệnh hệ thống. Khi bị kết hợp thành chuỗi khai thác, kẻ tấn công có thể gửi yêu cầu độc hại đến thiết bị chuyên dụng VPN để thực thi lệnh mà không cần xác thực.
...
- **Năm công bố:** 2024
- **Loại lỗi:** Chèn lệnh hệ thống, thực thi mã từ xa không cần xác thực
- **Nguồn chính thống:** https://security.paloaltonetworks.com/CVE-2024-3400

**Mô tả:**  
PAN-OS có lỗi chèn lệnh hệ thống trong GlobalProtect. Trong cấu hình bị ảnh hưởng, kẻ tấn công chưa xác thực có thể lợi dụng lỗi tạo file tùy ý và thực thi lệnh với quyền root trên tường lửa.
...
- Lỗi bị khai thác thực tế nên rủi ro rất cao.
...
- **Năm công bố:** 2023
- **Loại lỗi:** Tràn bộ đệm vùng heap, thực thi mã từ xa
- **Nguồn chính thống:** https://www.fortiguard.com/psirt/FG-IR-23-097

**Mô tả:**  
FortiOS SSL-VPN có lỗi tràn bộ đệm vùng heap. Attacker từ xa có thể gửi yêu cầu độc hại đến SSL-VPN để thực thi mã hoặc lệnh tùy ý.
...
- Rủi ro cao vì SSL-VPN thường được mở truy cập ra internet.
- Có thể dẫn đến đánh cắp thông tin xác thực hoặc triển khai malware.
...
- Kiểm tra log SSL-VPN và tài khoản bất thường sau khi vá.
...
- **Năm công bố:** 2024
- **Loại lỗi:** Đọc tệp tùy ý, lỗi bộ phân tích lệnh CLI
- **Nguồn chính thống:** https://www.jenkins.io/security/khuyến cáo bảo mật/2024-01-24/

**Mô tả:**  
Jenkins CLI dùng thư viện args4j hỗ trợ cú pháp `@file`, cho phép đọc nội dung file để thay thế argument. Jenkins để tính năng này bật mặc định, dẫn đến kẻ tấn công có thể đọc các file tùy ý trên Jenkins controller tùy theo quyền và lệnh CLI.
...
- Ảnh hưởng lớn vì Jenkins thường chứa thông tin xác thực CI/CD quan trọng.

**Giải pháp khắc phục:**  

- Nâng cấp Jenkins lên:
  - Jenkins weekly `2.442` hoặc mới hơn
  - Jenkins LTS `2.426.3` hoặc mới hơn
  - Jenkins LTS `2.440.1` hoặc mới hơn
...
- **Năm công bố:** 2025
- **Loại lỗi:** Bộ điều khiển ingress của Kubernetes, thực thi mã từ xa
- **Nguồn chính thống:** https://kubernetes.io/blog/2025/03/24/ingress-nginx-cve-2025-1974/

**Mô tả:**  
CVE-2025-1974 ảnh hưởng đến `ingress-nginx`. Lỗi cho phép kẻ tấn công có quyền truy cập mạng pod khai thác admission controller và có thể dẫn đến chiếm quyền trong cluster.
...
- Rủi ro cao với môi trường đa thuê bao hoặc cluster có workload không tin cậy.
...
- **Năm công bố:** 2024
- **Loại lỗi:** Chiếm quyền tài khoản, lỗi logic đặt lại mật khẩu
- **Nguồn chính thống:** https://docs.gitlab.com/releases/patches/patch-release-gitlab-16-7-2-released/

**Mô tả:**  
GitLab có lỗi trong quá trình đặt lại mật khẩu, cho phép email reset password được gửi đến địa chỉ email chưa được xác minh trong một số điều kiện. Điều này có thể dẫn đến chiếm quyền tài khoản nếu kẻ tấn công lợi dụng được luồng reset.
...
- Nâng cấp GitLab lên một trong các bản vá:
  ...
  - `16.6.4`
  - `16.7.2`
- Nếu nghi ngờ tài khoản bị xâm nhập/chiếm quyền:
  - Bắt buộc bật 2FA cho toàn bộ người dùng.
  - Rotate bí mật hệ thống, token và thông tin xác thực.
